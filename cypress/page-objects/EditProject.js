...
export default class EditProject extends BasePage {
    ...
    static openExistingProjectPage() {
        cy.get(".project-card__name").contains("גב האומה").click()
    }
    ...
    static editProjectDescription(editedDescription) {
        cy.get(".create-project-general__description-box").clear().type(editedDescription)
    ...
    static verifyChanges() {
        cy.get(".project-page-details-title").should("have.text", this.EDITED_PROJECT_NAME)
        cy.get(".project-page-details-unit").should("have.text", this.EDITED_PROJECT_FORCES)
        cy.get(".project-page-details-description").should("have.text", this.EDITED_PROJECT_DESCRIPTION)
    }
}